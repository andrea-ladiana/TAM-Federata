...
# -*- coding: utf-8 -*-
"""
Experiment 04 — Quantizzazione di J locali: valori finali vs bitwidth
===================================================================
...
- "shuffle" = stima di K_eff tramite baseline a permutazione (no Marchenko–Pastur)

Output:
- hyperparams.json
- log.jsonl (una riga per seed e per bitwidth)
- results_table.csv (tabella comoda)
- fig_summary.png (Seaborn 2×3 con bande SE)
...
# ---------------------------------------------------------------------
# Import moduli di progetto (file è in: <UNSUP>/stress_tests/)
# ---------------------------------------------------------------------
...
class HyperParams:
    ...
    bits_list: List[int] = field(default_factory=list)  # verrà settato nel main o in main()
...
def safe_propagate(J: np.ndarray, *, iters: int = 1, max_steps: int = 300, clip: float = 1e3) -> np.ndarray:
    from Functions import propagate_J  # import locale per evitare cicli
...
def quantize_symmetric(J: np.ndarray, bits: int, stochastic: bool, rng: np.random.Generator) -> Tuple[np.ndarray, float]:
    ...
    Ritorna (J_q, scale), dove 'scale' è lo step (o alpha per bits=1).
    """
    if bits <= 0:
