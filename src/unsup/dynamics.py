...
Il codice Ã¨ copiato (con minime aggiunte di docstring / import difensivi) per poter
...
- Le variabili con lettere greche (Î¾) sono state lasciate inalterate come nel notebook.
- Manteniamo anche funzioni "private" con underscore perchÃ© comparivano nel notebook.
...
try:
    ...
    TAM_Network = None  # verrÃ  controllato a runtime quando necessario
...
def new_round(data, round: int = 0, collection: bool = False):
    ...
    Nel notebook l'asse Ã¨ (L, n_batch, M_batch, N) ma la funzione era usata su ETA_unsup.
...
def disentangling(x: np.ndarray, K: int, L: int, J_rec: np.ndarray, JKS_iter: np.ndarray,
                  Î¾: np.ndarray, updates: int, Î¾r_old: Any = -10, Î²T: float = 2.5, h: float = 0.1,
                  Î»: float = 0.2, show_bar: bool = True):
    ...
    Ritorna: (Î¾r, ms) con pattern ricostruiti filtrati e magnetizzazioni massime.
    """
    ...
    Ïƒr = tf.convert_to_tensor([x] * L)
    Ïƒr = np.swapaxes(Ïƒr, 0, 1)
    Net = TAM_Network()
    Net.prepare(J_rec, L)
    Net.dynamics(Ïƒr, Î²T, Î», h, updates=updates, show_progress=show_bar, desc="TAM round")
    if Net.Ïƒ is None:
        return np.empty((0, N)), np.array([])
    Î¾r_new = np.reshape(np.asarray(Net.Ïƒ), (L * x.shape[0], N))
    if np.mean(Î¾r_old) != -10:
        Î¾r = np.vstack((Î¾r_new, Î¾r_old))
    else:
        Î¾r = Î¾r_new
    Î¾r_pre_prune = np.copy(Î¾r)  # backup per fallback
    # Check eigenvectors of KS
    temp = tf.einsum('ij,Aj->Ai', JKS_iter, Î¾r)
    check2 = tf.einsum('Ai,Ai->A', Î¾r, temp) / N
    toremove2 = (check2 < 0.6)
    if np.any(toremove2):
        Î¾r = np.delete(Î¾r, toremove2, axis=0)
    # mutual overlap pruning
    qÎ¾r = np.abs(np.array(tf.einsum('ai,bi->ab', Î¾r, Î¾r) / N))
    qÎ¾r = np.triu(qÎ¾r, 1)
    check = (qÎ¾r > 0.4)
    toremove = []
    for _ in range(Î¾r.shape[0]):
        wh = np.where(check[_])[0]
        if len(wh) > 0:
            for a in range(len(wh)):
                toremove.append(wh[a])
    if toremove:
        Î¾r = np.delete(Î¾r, toremove, axis=0)
    # Fallback se tutto rimosso
    if Î¾r.shape[0] == 0:
        Î¾r = Î¾r_pre_prune[: max(1, min(K, Î¾r_pre_prune.shape[0]))]
    # Calcolo magnetizzazioni (gestione edge-case singolo pattern)
    if Î¾r.shape[0] > 0:
        ms = np.max(np.abs(tf.einsum('ai,bi->ab', Î¾r, Î¾) / N), axis=1)
...
def dis_check(autovett: np.ndarray, K: int, L: int, J_rec: np.ndarray, JKS_iter: np.ndarray,
              Î¾: np.ndarray, updates: int = 80, show_bar: bool = True):
    """Wrapper iterativo che continua a chiamare disentangling finchÃ© non ottiene >= K magnetizzazioni."""
    K_eff, N = autovett.shape
    Î¾r, ms = disentangling(autovett, K, L, J_rec, JKS_iter, Î¾, updates, Î¾r_old=-10, show_bar=show_bar)
    attempts = 0
    max_attempts = 10
    while len(ms) < K and attempts < max_attempts:
        attempts += 1
        Î¾r, ms = disentangling(autovett, K, L, J_rec, JKS_iter, Î¾, updates, Î¾r_old=Î¾r, show_bar=show_bar)
...
# ---------------------------------------------------------------------------
# Sezione: Metriche e supporto per grid-search / stabilitÃ 
# ---------------------------------------------------------------------------
...
def _stability_across_seeds(pattern_sets: List[np.ndarray], true: np.ndarray) -> float:
    """Stima stabilitÃ  archetipi: media overlap pairwise dopo matching (0..1)."""
...
def run_single_experiment(base_params: Dict[str, Any], w_value: float, seed: int) -> Dict[str, Any]:
    ...
    Î¾_true = gen_patterns(N, K)
    J_star = JK_real(Î¾_true)
    ETA_unsup = gen_dataset_unsup(Î¾_true, M_unsup, r_ex, n_batch, L)
    ...
    for b in range(n_batch):
        ...
        if b == 0:
            ...
            J_hebb_prev = unsupervised_J(tf.convert_to_tensor(Î¾r_ref, dtype=tf.float32), 1)
        ...
        Î¾r_extend, Magn_extend = dis_check(autov_ext, K, L, J_rec_extend, JKS_iter_extend, Î¾=Î¾_true, updates=up, show_bar=False)
        Î¾r_ref = Î¾r_extend
        ...
        if b == n_batch - 1:
            patterns_extend_final = Î¾r_extend
    ...
    Î¾r_first, Magn_first = dis_check(autov0, K, L, J_unsup_first, JKS_first, Î¾=Î¾_true, updates=up, show_bar=False)

    m_first, _, _ = _match_and_overlap(Î¾r_first, Î¾_true)
    m_final, overlaps_final, _ = _match_and_overlap(patterns_extend_final, Î¾_true)
...
def timed_partial_run(w_value: float, seed: int, rounds: int, *,
    ...
    for b in range(rounds):
        ...
        if b == 0:
            ...
            if (w_value < 1.0) and (Î¾r_ref is not None):
                tH0 = time.perf_counter(); J_hebb_prev = unsupervised_J(tf.convert_to_tensor(Î¾r_ref, dtype=tf.float32), 1); tH1 = time.perf_counter(); t_hebb = tH1 - tH0
        ...
        tD0 = time.perf_counter(); vals_ext, vecs_ext = np.linalg.eig(JKS_iter_extend); mask_ext = (np.real(vals_ext) > 0.5); autov_ext = np.real(vecs_ext[:, mask_ext]).T; Î¾r_extend, _ = dis_check(autov_ext, K_b, L_b, J_rec_extend, JKS_iter_extend, Î¾=xi_true, updates=updates_b, show_bar=False); tD1 = time.perf_counter(); Î¾r_ref = Î¾r_extend
